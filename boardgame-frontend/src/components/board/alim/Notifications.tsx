...
export default function Notifications({ initialData }: { initialData: Notification }) {
  ...
  async function loadMore() {
    ...
    try {
      ...
      setIsLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_SERVER_HOST}/api/notifications?page=${nextPage}&size=10`, {
        headers: {
    ...
    <>
      <ul className="px-5 flex flex-col gap-1">
        {notifications.map((data) => (
          <li key={data.id || data.resourceId} className="bg-white rounded-xl p-4 cursor-pointer">
